...
-- Created: 2025-01-07
-- Description: Sistema completo de convites para workspaces
...
CREATE TABLE IF NOT EXISTS workspace_invites (
  ...
  -- Constraints
  CONSTRAINT unique_pending_invite UNIQUE (email, workspace_id, status) DEFERRABLE INITIALLY DEFERRED
);
...
CREATE INDEX IF NOT EXISTS idx_workspace_invites_inviter_id ON workspace_invites(inviter_id);
...
BEGIN
  ...
  VALUES (invite_record.workspace_id, user_id, invite_record.role, NOW())
...
-- INSERT INTO workspace_invites (email, workspace_id, inviter_id, token, message)
-- SELECT 
--   'test@example.com',
--   w.id,
--   u.id,
...
DO $$
BEGIN
  RAISE NOTICE '✅ Tabela workspace_invites criada com sucesso';
